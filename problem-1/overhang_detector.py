...
class OverhangDetector(object):
    ...
    def __init__(self, overhang=np.deg2rad(45), down=(0, 0, -1)):
        ...
        self.down = np.array(down) / np.linalg.norm(down)
    ...
    def solveAndDisp(self, faces, showFaces=True, showEdges=True, showVertices=True):
        ...
        mins = np.min(vertices.keys(), axis=0)
        maxes = np.max(vertices.keys(), axis=0)

        axes.set_xlim(mins[0], maxes[0])
        axes.set_ylim(mins[1], maxes[1])
        axes.set_zlim(mins[2], maxes[2])
        ...
        if showFaces:
            facecollection = mplot3d.art3d.Poly3DCollection(faces, alpha=0.5)
...
if __name__ == '__main__':
    zdetector = OverhangDetector()

    zdetector.solveAndDisp(Mesh.from_file('tests/octahedron.stl').vectors)[0].show()
    zdetector.solveAndDisp(Mesh.from_file('tests/cube.stl').vectors)[0].show()

    ydetector = OverhangDetector(down=(0, -1, 0))
    ydetector.solveAndDisp(Mesh.from_file('part.stl').vectors, showVertices=False)[0].show()
